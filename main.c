...
int main()
{
    ...
    for (int loop1 = 1; loop1 == 1;)
    {
        ...
        {
            switch (instrumento) //faz a seleção dos arquivos do instrumento escolhido
            {
                 case '1':
                 printf ("||Instrumento atual: Violão||\n\n  Pressione as teclas de 1 a 7 para tocar.\n\n >Para selecionar outro instrumento, pressione 0<.\n");

                 for (int loop2 = 1; loop2 == 1;) //mantém o programa rodando o instrumento atual até que haja cancelamento
                     {nota = getch (stdin);
                      switch (nota)
                      {
